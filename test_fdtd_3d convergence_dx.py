...
# %% convergence test for dr

dr = np.linspace(15*1e-9,50*1e-9,13) # initial 30e-9
# print(dr)
...
for l, dri in enumerate(dr):
    start = time.time()
    # x coordinates
    x = np.arange(-int(np.ceil((Nx-1)/2)), int(np.floor((Nx-1)/2)) + 1)*dri
    # y coordinates
    y = np.arange(-int(np.ceil((Ny-1)/2)), int(np.floor((Ny-1)/2)) + 1)*dri
    source_width = 2*dri # width of Gaussian current dist. [grid points]
    jx = np.zeros((Nx, Ny, Nz))
    jy = np.zeros((Nx, Ny, Nz))
    jz = np.zeros((Nx, Ny, Nz)) 
    for i, xi in enumerate(x):
...
plt.figure()
plt.plot(dr*1e9, operation_time, 'o-')
